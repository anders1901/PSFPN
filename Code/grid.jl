...
# Python libraries
np = pyimport("numpy")
# plt.contour is used instead of contour Julia
...
function trace_figure(A, eps)
    ...
    A : Array{Complex{Float64},n} matrix of size n x n
    eps : Float64 limit
    return circles : Array{Any,1} array of centers Complex{Float64} and rays Complex{Float64}
           p : plot object
    =#
    eig = eigvals(A)
...
# Function used to construct and/ or plot the contour window for grid
function build_contour(dots, p; plot_ = true)
    ...
    return [x y w h] : Array{Any,4}  array of coordinates of window for GRID
           p : plot object
    =#

    # We get the centers and rays from the dots array
...
function plot_pseudospectre(A, eps; plot_ = true)
    ...
    p : plot
    plot_ = true : ploting option if false then we only return the window coordinates
    return contour_ : Array{Any,4}  array of coordinates of window for GRID
           p : plot object
    =#
    @time begin
